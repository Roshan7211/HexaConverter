import 'server-only';

import { cookies } from 'next/headers';

import { JobStatus, type PlanTier } from '@prisma/client';
import { getServerSession } from 'next-auth';

import { authOptions } from '@/services/auth/auth-options';
import { serverEnv } from '@/lib/env';
import {
  limitsFor,
  USAGE_PERIOD_DAYS,
  type EffectiveLimits,
} from '@/lib/plans';
import * as jobs from '@/database/repositories/job.repository';
import {
  createGuestId,
  GUEST_COOKIE,
  GUEST_COOKIE_MAX_AGE,
  isValidGuestId,
} from '@/lib/security';

/**
 * Requester identity and quota enforcement.
 *
 * Signed-in users are identified by their session; everyone else gets an
 * opaque, http-only guest cookie so anonymous conversions can be listed and
 * rate-limited without any account or tracking identifier.
 */

export interface Requester {
  userId: string | null;
  guestId: string | null;
  plan: PlanTier | null;
  /** Stable owner key used for storage tickets and quota accounting. */
  ownerKey: string;
  limits: EffectiveLimits;
  isAuthenticated: boolean;
}

/**
 * Resolves the current requester, creating a guest cookie when needed.
 * Only callable from route handlers and server actions, where cookies are
 * writable.
 */
export async function resolveRequester(): Promise<Requester> {
  const session = await getServerSession(authOptions);

  if (session?.user?.id) {
    return {
      userId: session.user.id,
      guestId: null,
      plan: session.user.plan,
      ownerKey: `u:${session.user.id}`,
      limits: limitsFor(session.user.plan),
      isAuthenticated: true,
    };
  }

  const store = await cookies();
  const existing = store.get(GUEST_COOKIE)?.value;

  let guestId: string;
  if (isValidGuestId(existing)) {
    guestId = existing;
  } else {
    guestId = createGuestId();
    store.set(GUEST_COOKIE, guestId, {
      httpOnly: true,
      sameSite: 'lax',
      secure: serverEnv().NODE_ENV === 'production',
      path: '/',
      maxAge: GUEST_COOKIE_MAX_AGE,
    });
  }

  return {
    userId: null,
    guestId,
    plan: null,
    ownerKey: `g:${guestId}`,
    limits: limitsFor(null),
    isAuthenticated: false,
  };
}

/** Read-only variant for server components, which cannot set cookies. */
export async function peekRequester(): Promise<Requester | null> {
  const session = await getServerSession(authOptions);

  if (session?.user?.id) {
    return {
      userId: session.user.id,
      guestId: null,
      plan: session.user.plan,
      ownerKey: `u:${session.user.id}`,
      limits: limitsFor(session.user.plan),
      isAuthenticated: true,
    };
  }

  const store = await cookies();
  const existing = store.get(GUEST_COOKIE)?.value;
  if (!isValidGuestId(existing)) return null;

  return {
    userId: null,
    guestId: existing,
    plan: null,
    ownerKey: `g:${existing}`,
    limits: limitsFor(null),
    isAuthenticated: false,
  };
}

/** Narrows the requester to the owner scope the repositories expect. */
export function ownerScope(requester: Requester) {
  return { userId: requester.userId, guestId: requester.guestId };
}

export interface QuotaVerdict {
  allowed: boolean;
  reason?: string;
  used: number;
  limit: number;
}

/**
 * Checks the rolling conversion allowance. Signed-in usage is tracked on the
 * user record; guest usage is counted from their job history.
 */
export async function checkQuota(requester: Requester): Promise<QuotaVerdict> {
  const limit = requester.limits.jobsPerPeriod;
  const periodStart = new Date(
    Date.now() - USAGE_PERIOD_DAYS * 24 * 60 * 60 * 1000,
  );

  const used = await jobs.countForOwner(ownerScope(requester), {
    createdAt: { gte: periodStart },
    status: { not: JobStatus.CANCELLED },
  });

  if (used >= limit) {
    return {
      allowed: false,
      used,
      limit,
      // There is nothing to upsell: the allowance is the same for everyone, so
      // the honest message is when it resets, not what to buy.
      reason: `You have used all ${limit.toLocaleString()} conversions in the last ${USAGE_PERIOD_DAYS} days. The allowance is a rolling window, so it frees up as older conversions age out.`,
    };
  }

  return { allowed: true, used, limit };
}

/** Enforces the plan's concurrent-conversion ceiling. */
export async function checkConcurrency(
  requester: Requester,
): Promise<{ allowed: boolean; reason?: string }> {
  const active = await jobs.countForOwner(ownerScope(requester), {
    status: { in: [JobStatus.QUEUED, JobStatus.PROCESSING] },
  });

  if (active >= requester.limits.concurrentJobs) {
    return {
      allowed: false,
      reason: `You already have ${active} conversion${active === 1 ? '' : 's'} in progress. Wait for ${
        requester.limits.concurrentJobs === 1 ? 'it' : 'them'
      } to finish before starting another.`,
    };
  }

  return { allowed: true };
}

/** Retention deadline for a new job, from the requester's plan. */
export function retentionDate(requester: Requester): Date {
  const hours = Math.min(
    requester.limits.retentionHours,
    serverEnv().FILE_RETENTION_HOURS,
  );
  return new Date(Date.now() + hours * 60 * 60 * 1000);
}
